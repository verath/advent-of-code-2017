...
def count_inf_cycles(memBankStr):
    history = []
    ...
    while True:
        ...
        if mem_banks in history:
            return len(history) - history.index(mem_banks)
        history.append(mem_banks[:])

def main():
    assert count_inf_cycles(EXAMPLE_INPUT) == 4
    with open('input.txt', 'r') as input_file:
        print(count_inf_cycles(input_file.read()))
